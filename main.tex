...
Let us consider a manifold $ \mathbb{R}_{\geq} \times \mathcal{M} $, where
$\matcal{M} $ is Riemannian mainfold. We will define a Riemannian tensor on this product
by $dt \otimes dt + f^{2}(t)g $, where $g$ is the metric on $\matchcal{B}$
% Rysuneczek z rurkom
\[
    T_{(t, m)} = \mathbb{R} \times T_m \mathcal{M}
\]
% Rysuneczek kwadrata

\[
\omega_t \in \Lambda^k(\mathbb{R} \oplus T_m \mathcal{M}) = 
\]
In other words every $k$-form can be written as $\omega = \eta + dt \wedge \xi$,
where both $\eta$ and $\xi$ do not contain $dt$.
Please note that $\eta$ is $k
